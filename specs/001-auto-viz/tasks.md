...
## Phase 4: User Story 2 - 데이터소스 연결 설정 (Priority: P2) ✅ COMPLETE
...
- [x] T054 [US2] Implement DataSource CRUD service (create, read, update, delete) in src/services/storage/dataSourceService.ts
- [x] T055 [US2] Implement connection test service (testConnection with status update) in src/services/prometheus/connectionTest.ts

### DataSource Hooks

- [x] T056 [US2] Implement useDataSources hook (list, active, save, delete) in src/hooks/useDataSources.ts

### DataSource UI

- [x] T057 [P] [US2] Create DataSourceForm component (URL + auth token inputs) in src/components/DataSource/DataSourceForm.tsx
- [x] T058 [P] [US2] Create ConnectionStatus indicator component in src/components/DataSource/ConnectionStatus.tsx
- [x] T059 [P] [US2] Create DataSourceList component (saved datasources) in src/components/DataSource/DataSourceList.tsx
- [x] T060 [US2] Create DataSourcePanel container in src/components/DataSource/DataSourcePanel.tsx
- [x] T061 [US2] Create DataSource re-export in src/components/DataSource/index.ts

### Integration

- [x] T062 [US2] Replace temporary URL input with DataSourcePanel in src/App.tsx
- [x] T063 [US2] Connect active datasource to query execution in src/hooks/usePrometheusQuery.ts

**Checkpoint**: ✅ 데이터소스 저장/불러오기 동작

---

## Phase 5: User Story 3 - 시간 범위 및 Step 설정 (Priority: P3) ✅ COMPLETE
...
- [x] T064 [P] [US3] Implement time range presets (last 1h, 6h, 24h, 7d) in src/utils/timeRangePresets.ts
- [x] T065 [P] [US3] Implement time range resolver (relative to absolute) in src/utils/timeRangeResolver.ts
- [x] T066 [US3] Implement step validator (warn if too many data points) in src/utils/stepValidator.ts

### Time Range UI

- [x] T067 [P] [US3] Create TimeRangeSelector component (preset dropdown + custom inputs) in src/components/QueryEditor/TimeRangeSelector.tsx
- [x] T068 [P] [US3] Create StepInput component with validation in src/components/QueryEditor/StepInput.tsx
- [x] T069 [US3] Integrate TimeRangeSelector and StepInput into QueryEditor in src/components/QueryEditor/QueryEditor.tsx

### Integration

- [x] T070 [US3] Update usePrometheusQuery to use time range and step parameters in src/hooks/usePrometheusQuery.ts

**Checkpoint**: ✅ 시간 범위/step 변경 시 쿼리 결과 변화 확인

---

## Phase 6: User Story 4 - 쿼리 오류 처리 (Priority: P4) ✅ COMPLETE
...
- [x] T071 [US4] Implement error classifier (network, cors, auth, query, timeout) in src/services/prometheus/errorClassifier.ts
- [x] T072 [US4] Implement CORS error detector in src/utils/corsDetector.ts

### Error UI

- [x] T073 [P] [US4] Create ConnectionErrorDisplay (network/CORS errors with troubleshooting) in src/components/common/ConnectionErrorDisplay.tsx
- [x] T074 [P] [US4] Create QueryErrorDisplay (PromQL syntax errors) in src/components/common/QueryErrorDisplay.tsx
- [x] T075 [P] [US4] Create EmptyResultDisplay (no data message with suggestions) in src/components/common/EmptyResultDisplay.tsx
- [x] T076 [P] [US4] Create TimeoutErrorDisplay (timeout with retry option) in src/components/common/TimeoutErrorDisplay.tsx

### Integration

- [x] T077 [US4] Integrate error displays into App based on error type in src/App.tsx
- [x] T078 [US4] Add retry functionality for timeout errors in src/hooks/usePrometheusQuery.ts
...
- [x] T080 [P] Add responsive layout styles in src/index.css
- [ ] T081 [P] Add dark mode support (optional) in tailwind.config.js

### Performance

- [x] T082 Implement query result caching with TanStack Query in src/hooks/usePrometheusQuery.ts
- [x] T083 Add large dataset warning (>10k points) in src/components/Visualization/VisualizationPanel.tsx

### Documentation

- [x] T084 [P] Update README.md with usage instructions
- [ ] T085 [P] Add inline JSDoc comments to public functions in src/services/

### Final Validation

- [x] T086 Run full quickstart.md validation flow
...
| Phase 4: US2 | ✅ Complete | 10 | 10 |
| Phase 5: US3 | ✅ Complete | 7 | 7 |
| Phase 6: US4 | ✅ Complete | 8 | 8 |
| Phase 7: Polish | ✅ Complete | 8 | 10 |
| **Total** | ✅ **86/88** | **86** | **88** |
...
- **전체 구현 완료**: Phase 1-7 ✅ (선택적 태스크 2개 제외)
